...
for a=1:length(manners)
    for b=1:length(paths)
        ...
        for obj = 1:1
            img = imread([currentFolder '/img/' num2str(obj) '.jpg'],'JPEG'); %read in object %imread gives 3 matrices: r, g, b
            ...
            if strcmp(mode,'movie')
                ...
                %instead.
                % 
                x = x+50; %because image is 100 by 100 square (so you want midpoint)
                y = y+50;
                ...
                for j=1:30 %30 frames; draws all movie frames into movie file %j is an iterator
                    image(newimg);
                    axis off;
                    m(j) = getframe; %m is what's being displayed
                    drawnow;
                end
                ...
                for i = 1:lens
                    %rotate the triangle
                    img_rot = imrotate(img, rotations(i)); %rotations is rotation matrix %increases size of image because upright square containing it is larger
